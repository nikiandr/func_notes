...
\noindent\ref{N:1_7_9}. Оператор $A$ компактний. Якщо $\dim H \geq 2$, то $\mathrm{Ker} A = \{0\}$ і $\sigma (A) = \sigma_T (A)$.
 ...
 Резольвенту $R_{\lambda} (A)$ при $\lambda \notin \{0; (x_1, x_0)\}$ шукаємо як розв'язок рівняння: $\lambda y - \dotprod{y}{x_0} x_1 = x$.
 Покладемо: $x = x' + x''$, де $x' \in$ л. о. $\{x_1\}$; $x'' \perp x_1$; $y = \alpha x_1 + y''$, де $y'' \perp x_{\perp}$;
 $x'' = x - \dotprod{x}{x_{\perp}}x_{\perp}$. Маємо: $y'' = \frac{1}{\lambda} x''$; 
...
\noindent\ref{N:1_7_14}. а) Якщо $\lambda \notin [0; 1]$, то рівняння $(\lambda - t)y(t) = x(t)$ має в $L_2 [0; 1]$
 ...
 Якщо $\lambda \in [0; 1]$, то рівняння $(\lambda - t)y(t) = \frac{1}{(\lambda - t)^{\frac{1}{3}}} \in L_2[0; 1]$ немає розв'язку в $L_2[0,1]$,
...
\noindent\ref{N:1_7_30}. б) Припустимо спочатку, що $M > 0$ та $M > |m|$. Тоді $M = \sup\limits_{\norm{x} = 1} \dotprod{Ax}{x} = \sup\limits_{\norm{x} = 1} |\dotprod{Ax}{x}| = \norm{A} = r(A)$.
...
в $X = L_2 [0; 1]: 0 \in \sigma_{\text{к}} (A)$, тому що $(\int\limits_0^t x(s) ds = 0 \text{ в } L_2[0; 1]) \Rightarrow$
...
\noindent\ref{N:2_1_10}. Для перевірки необмеженості оператора $A$ розгляньте послідовність $x_n (t) = \sin (nt)$.
...
$D(A) = \{u$ --- поліном; $u(0) = 0\}$; $\Delta$ --- числовий відрізок $[a; b] \subset [0; 1]$; $j_{\Delta}$ --- індикатор $\Delta$.